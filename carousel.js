/*
 * SDL Collection Carousel v1.0
 * Fetches any Squarespace collection (Blog, Portfolio, Events, Shop) via the
 * Squarespace JSON API and renders it as a Swiper-powered touch slider.
 *
 * Written without classes/constructors: each carousel is a plain state object
 * (`cc`) passed through a set of pure-ish functions. Swiper 11.2.1 is bundled
 * at the bottom of this file and exposed locally as `Swiper`.
 *
 * Identifier: data-sdl-plugin="collection-carousel"
 */
(function () {
  "use strict";
  ...
  function getUserSettings() {
    return window["sdlCollectionCarouselSettings"] || {};
  }
  ...
  function deepMerge() {
    ...
    let clone = {};
    objs.forEach(obj => mergeObj(clone, obj));
    return clone;
  }
  ...
  /* ------------------------------------------------------------------ *
   *  Per-element data attribute parsing
   * ------------------------------------------------------------------ */
  function getInstanceSettings(el) {
    ...
    if (el.dataset.effect === "creative-coverflow") {
      ...
      el.dataset.creative = JSON.stringify({
        limitProgress: 3,
        progressMultiplier: 1,
        prev: { opacity: 0.75, translate: ["-110%", 0, 0], scale: 0.9, rotate: [0, -10, 0] },
        next: { opacity: 0.75, translate: ["110%", 0, 0], scale: 0.9, rotate: [0, 10, 0] },
      });
    }
    ...
    if (el.dataset.effect === "creative-coverflow-2") {
      ...
      el.dataset.creative = JSON.stringify({
        limitProgress: 1,
        progressMultiplier: 1,
        prev: { opacity: 1, translate: ["-80%", 0, 0], scale: 0.9, rotate: [0, 0, 0] },
        next: { opacity: 1, translate: ["80%", 0, 0], scale: 0.9, rotate: [0, 0, 0] },
      });
    }
    ...
    ["metadataBelowExcerpt", "metadataAboveTitle", "metadataBelowTitle"].forEach(key => {
      if (typeof dataAttributes[key] === "string") {
        dataAttributes[key] = dataAttributes[key]
          .split(",")
   ...
   * ------------------------------------------------------------------ */
  function getSwiperConfig(settings, container) {
    const handleSlideWidth = value => {
      ...
      if (typeof value === "string" && value.match(/\d+(\.\d+)?(px|rem|em|vw|%)/)) {
        return { slidesPerView: "auto", width: value };
      }
      return { slidesPerView: parseFloat(value), width: null };
    };
    ...
    let mdView = handleSlideWidth(settings.slidesPerViesdld || settings.slidesPerView);
    ...
    const getGroupSize = (groupValue, fallbackGroupValue, viewValue) => {
      if (settings.groupSlides) {
        ...
        return Number.isFinite(parsedView) ? Math.max(1, Math.floor(parsedView)) : 1;
      }
    ...
    const updateSlideWidths = swiper => {
      ...
      mdView = handleSlideWidth(settings.slidesPerViesdld || settings.slidesPerView);
      lgView = handleSlideWidth(settings.slidesPerViewLg || settings.slidesPerView);

      const width = window.innerWidth;
      let currentView;
      if (width >= 1024) currentView = lgView;
      else if (width >= 768) currentView = mdView;
      else currentView = smView;

      if (currentView.width) {
        swiper.slides.forEach(slide => (slide.style.maxWidth = currentView.width));
      } else {
        swiper.slides.forEach(slide => (slide.style.width = ""));
      }
      swiper.update();
    };
    ...
    return {
      ...
      navigation: {
        ...
        prevEl: settings.freeMode
          ...
          : `#${settings.id} .navigation-button-prev`,
      },
      pagination: {
      ...
      breakpoints: {
        ...
        768: {
          ...
          slidesPerGroup: getGroupSize(
            ...
            settings.slidesPerGroup,
            settings.slidesPerViesdld || settings.slidesPerView
          ),
        },
        1024: {
      ...
      on: {
        ...
        init: function (swiper) {
          ...
          // freeMode + navigation needs a custom prev handler (Swiper's default
          // slidePrev does not snap correctly in freeMode).
  ...
  /* ------------------------------------------------------------------ *
   *  Per-item content builders
   *  Each builder returns a DOM node (or null) for a single item.
   * ------------------------------------------------------------------ */
  function createBuilders(cc) {
    ...
    const builders = {
      tags: item => {
        ...
        const isLink = settings.metadataTagLinks;
        tagList.innerHTML = item.tags
          .map(
            (tag, index, array) => `
                <li>
                    <span class="tag" rel="tag">${isLink ? `<a href="${cc.collection.fullUrl}/tag/${convertToSlug(tag)}">` : ""}${tag}${isLink ? `</a>` : ""}${
              index < array.length - 1 && settings.tagsDelimiter === ", "
          ...
          .join(
            settings.tagsDelimiter === ", "
              ? " "
              : `<span class="tag-delimiter">${settings.tagsDelimiter}</span>`
          );
      ...
      categories: item => {
        ...
        const isLink = settings.metadataCategoryLinks;
        if (item.categories?.length) {
          categoryList.innerHTML = item.categories
            .map(
              (category, index, array) => `
              <li>
                <span class="category" rel="category">${isLink ? `<a href="${cc.collection.fullUrl}/category/${convertToSlug(category)}">` : ""}${category}${isLink ? `</a>` : ""}${
                index < array.length - 1 && settings.categoriesDelimiter === ", "
            ...
            .join(
              settings.categoriesDelimiter === ", "
                ? " "
                : `<span class='category-delimiter'>${settings.categoriesDelimiter}</span>`
            );
      ...
      publisheddate: item => {
        ...
        const dateFormat = settings.eventDateFormat;
        const options = {
          ...dateFormat.options,
      ...
      publishdate: function (item) {
        return builders.publisheddate(item);
      },
      date: function (item) {
        return builders.publisheddate(item);
      },
      ...
      price: item => {
        if (cc.type !== "products") return null;
        ...
        if (variants.length > 0) {
          ...
          const formatPrice = amount =>
            symbolIsPrefix ? `${currencySymbol}${amount}` : `${amount} ${currencySymbol}`;
      ...
      eventDates: item => {
        ...
        const formatDate = (date, includeDate = true) => {
          if (includeDate) return date.toLocaleString(dateFormat.locale, options);
        ...
        eventDatesContent.innerHTML = sameDay
  ...
  /* ------------------------------------------------------------------ *
   *  Collection data fetching (Squarespace JSON API)
   * ------------------------------------------------------------------ */
  async function getCollectionData(cc) {
    const sourceUrl = cc.el.dataset.source;
    let limit = cc.settings.limit;
    if (!sourceUrl) return { items: [], type: null };
    let items = [];
    let collection = null;
    let type = null;
    ...
    const getCacheKey = source => `sdlPlugins.${source.replace(/[^a-zA-Z0-9]/g, "_")}`;
    const getCacheMetaKey = cacheKey => `${cacheKey}_meta`;
    ...
    const isCacheValid = metaKey => {
      ...
      return cacheAge < cc.settings.cacheDuration;
    };
    ...
    const getData = async path => {
      try {
        ...
        if (!response.ok) {
          if (response.status === 404) throw new Error(`Collection not found (404): ${path}`);
        ...
        if (type === "event") {
          if (!cc.el.dataset.limit) {
            cc.settings.limit = 40;
            limit = 40;
          }
          const upcoming = json.upcoming.map(item => ({ ...item, upcoming: true }));
          ...
          if (nestedCategories) {
            currentItems.forEach(item => {
              item.categoriesFull = item.categoryIds
                .map(categoryId =>
                  nestedCategories.categories.find(cat => cat.id === categoryId) || null
                )
        ...
        if (json.pagination?.nextPage) {
          await getData(appendQueryParams(json.pagination.nextPageUrl, { format: "json" }));
        }
        return { items, type, collection };
      } catch (error) {
    ...
    try {
      ...
      if (cc.settings.cacheDuration > 0) {
      ...
      const freshData = await getData(appendQueryParams(sourceUrl, { format: "json" }));

      if (cc.settings.cacheDuration > 0) {
        localStorage.setItem(cacheKey, JSON.stringify(freshData));
        localStorage.setItem(cacheMetaKey, JSON.stringify({ timestamp: new Date().getTime() }));
      }
  ...
  function buildStructure(cc) {
    ...
    cc.swiperWrapper = swiperWrapper;
    swiperWrapper.className = "swiper-wrapper";

    if (cc.siteJson) {
      if (cc.siteJson.pagePadding) {
        cc.el.style.setProperty("--sdl-cc-page-padding", cc.siteJson.pagePadding);
      }
      if (cc.siteJson.maxPageWidth) {
        cc.el.style.setProperty("--sdl-cc-max-page-width", cc.siteJson.maxPageWidth);
      }
    }

    // Wire the configured thumbnail aspect ratio to the CSS variable used by carousel.css.
    if (settings.aspectRatio && settings.aspectRatio !== "auto") {
      cc.el.style.setProperty("--sdl-cc-aspect-ratio", settings.aspectRatio);
    }

    if (cc.type === "event" && settings.events === "upcoming") {
      cc.items = cc.items.filter(item => item.upcoming);
    } else if (cc.type === "event" && settings.events === "past") {
      cc.items = cc.items.filter(item => !item.upcoming);
    }
    ...
    if (settings.loop) {
      ...
      const groups = [
        getEffectiveGroupSize(settings.slidesPerGroup, settings.slidesPerGroup, settings.slidesPerView),
        getEffectiveGroupSize(settings.slidesPerGroupSm, settings.slidesPerGroup, settings.slidesPerViewSm || settings.slidesPerView),
        getEffectiveGroupSize(settings.slidesPerGroupMd, settings.slidesPerGroup, settings.slidesPerViesdld || settings.slidesPerView),
        getEffectiveGroupSize(settings.slidesPerGroupLg, settings.slidesPerGroup, settings.slidesPerViewLg || settings.slidesPerView),
      ];
      const isUneven = groups.some(g => g > 1 && cc.items.length % g !== 0);
      const maxGroup = Math.max(...groups);
      if (isUneven) {
        const strategy = settings.groupSlidesLoopStrategy || "keep-blanks";
        const msg = `Collection carousel: ${cc.items.length} items not divisible by group size (${maxGroup}). Loop would add blank slides.`;
    ...
    if (settings.loop && settings.loopFillItems && cc.items.length > 0) {
      ...
      if (cc.items.length < minSlides) {
        const origItems = [...cc.items];
        while (cc.items.length < minSlides) {
          cc.items.push(...origItems);
        }
      }
    }

    const builders = createBuilders(cc);

    if (
      settings.loop &&
      !settings.loopFillItems &&
      settings.slidesPerViewLg + 1 > cc.items.length
    ) {
      const notification = document.createElement("div");
      notification.className = "collection-carousel-notification";
      notification.innerHTML = `<p>Not enough items to display ${settings.slidesPerViewLg} slides in view. Reducing the slides in view to ${cc.items.length - 1}. Add more items to display more slides in view.</p>
        <p>This message will only appear in the editor.</p>`;
      settings.slidesPerViewLg = cc.items.length - 1;
      cc.el.appendChild(notification);
    }
    ...
    cc.items.forEach(item => {
      ...
      if (settings.price && cc.type === "products") {
    ...
    cc.el.appendChild(swiperContainer);

    if (settings.navigation) {
      const navigationWrapper = document.createElement("div");
      cc.navigationWrapper = navigationWrapper;
      ...
      const arrowPrev = settings.navigationArrowPrev ?? defaultArrowPrev;
      const arrowNext = settings.navigationArrowNext ?? defaultArrowNext;

      const prevButton = document.createElement("button");
      cc.prevButton = prevButton;
      prevButton.innerHTML = `<div class="swiper-button-background"></div>${arrowPrev}`;
      prevButtonWrapper.appendChild(prevButton);

      const nextButton = document.createElement("button");
      cc.nextButton = nextButton;
      ...
      cc.el.appendChild(navigationWrapper);
    }
    ...
    if (settings.navigationLayout.includes("bottom") && settings.pagination) {
      cc.navigationWrapper.append(cc.pagination);
    }
  }
   ...
   *  Instance lifecycle
   * ------------------------------------------------------------------ */
  async function initCarousel(cc) {
    emitEvent("sdlCollectionCarousel:beforeInit", cc, cc.el);

    const { items, type, collection } = await getCollectionData(cc);
    ...
    cc.siteJson = window.Static?.SQUARESPACE_CONTEXT?.tweakJSON || {};

    buildStructure(cc);

    cc.swiperSettings = getSwiperConfig(cc.settings, cc.el);
    cc.swiper = new Swiper(cc.swiperContainer, cc.swiperSettings);

    emitEvent("sdlCollectionCarousel:ready", cc, cc.el);
    cc.el.sdlCollectionCarousel = {
  ...
  function createCarousel(el) {
    const cc = { el: el };
    cc.settings = deepMerge(
      {},
      getDefaultSettings(),
      getUserSettings(),
      getInstanceSettings(el)
    );
    if (!el.id) {
      el.id = `sdl-carousel-${Math.random().toString(36).substr(2, 9)}`;
    }
    cc.settings.id = el.id;
    initCarousel(cc);
    return cc;
  }
  ...
  /* ================================================================== *
   *  Bundled Swiper 11.2.1 (MIT) — exposed locally as `Swiper`
   * ================================================================== */
  ...
  /* ------------------------------------------------------------------ *
   *  DOM scanner / public API
   * ------------------------------------------------------------------ */
  function initCollectionCarousel() {
    const els = document.querySelectorAll(
      '[data-sdl-plugin="collection-carousel"]:not([data-loading])'
    );
  ...
  window.sdlCollectionCarousel = { init: () => initCollectionCarousel() };
